const handler = async (m, { conn, usedPrefix, command, args }) => {
  const features = [
    ...
    sections = features.map((f, i) => {
      const isActive = [
          ...
          ? activeFeatures.includes(f)
            ? !db.data.settings[conn.user.jid][f]
            : db.data.settings[conn.user.jid][f]
          : activeFeatures.includes(f)
            ? !db.data.chats[m.chat][f]
            : db.data.chats[m.chat][f],
        status = isActive ? "ON" : "OFF",
        rows = [
          {
            ...{
              header: `${(i + 1).toString().padEnd(2)}. ${f
                .replace(/([A-Z])/g, " $1")
                .replace(/^./, (str) => str.toUpperCase())
                .padEnd(18)}`,
              id: `${usedPrefix + command} ${i + 1}`,
              title: `- Switch ${statusLable} ${f
                .replace(/([A-Z])/g, " $1")
                .replace(/^./, (str) => str.toUpperCase())
                .padEnd(18)}\n- *Because*`,
    ...
    totalOn = Object.values(db.data.chats[m.chat]).filter(
      (val) => !0 === val,
    ).length,
    totalOff = Object.values(db.data.chats[m.chat]).filter(
      (val) => !1 === val,
    ).length,
    listEnab =
